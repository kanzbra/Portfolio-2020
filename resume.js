...
$(function() {
	...
	// Variables that hold whether an animation has been played
		...
		if (!circleAnimationHasPlayed) {
			...
			$(".canvas").each(function() {
				...
				var $this = $(this), /* This refers to the object, however 'this' cannot be used to obtain information from the object relatable to the canvas      											    element, because of the way that jQuery works */
					...
					percentage = $this.data("percent"), /* 'this' is used to obtain data information regarding the data-percent the user has coded in the html 																				document. */
				...
				function progress() {
					...
					paintbrush.fillText(
						...
						canvas_width / 2,
						canvas_height / 1.8
					); // Dynamic text that is positioned center within the canvas
					...
					paintbrush.arc(
						...
						canvas_height / 2.1,
						start_point,
						difference / 10 + start_point,
						false
					); // The starting point and arc of the drawing
					paintbrush.stroke(); // Begin drawing

					/* If the amount loaded is less than the percentage user defined variable from the html document, 
					   then the amount loaded will increase until it is. */
